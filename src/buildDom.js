...
module.exports = (allVideosArr) => {
  ...
  for (let i = 0; i < 16; i += 1) {
    ...
    const addElement = `${'<img class="video-container-img" '
            ...
            + `<i class="fas fa-user fa-2x" style="color: black"></i>${allVideosArr[i].snippet.channelTitle}`
            + '</li>'
            + ''
            + '<li class="video-container-ul-li">'
            + `<i class="fas fa-calendar-alt fa-2x" style="color: black"></i>${allVideosArr[i].snippet.publishedAt.substr(0, 10)}`
            + '</li>'
            + '<li class="video-container-ul-li">'
            + `<i class="fas fa-eye fa-2x" style="color: black"></i>${statVideosArr[i].statistics.viewCount}`
            + '</li>'
            + '</ul>'
